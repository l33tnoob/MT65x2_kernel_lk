...
#ifdef  USE_SINGLE_THREAD_FOR_SENDER
void  MediaSender::scheduleNextSend(){
	 sp<AMessage> msg = new AMessage(kWhatScheduleSend, id());
   	 msg->post();
}
status_t  MediaSender::onScheduleSend(){
    status_t err = OK;
    ...
    if (mMode == MODE_TRANSPORT_STREAM) {
        ...
        for (;;) {
		   ...
		   //send any data ready, avoid one track wait for another track , which will cause data blcok here
			...
			int32_t dummy;
			 if( accessUnit->meta()->findInt32("dummy-nal", &dummy) && dummy == 1){	
	    ...
	    if (err != OK) {
		ALOGE("packetizeAccessUnit fail");
                break;
            }
        }
    }	
...
char value[PROPERTY_VALUE_MAX];
   if((mFirstconstruct )  && (mLogFile == NULL))
   {
       struct sched_param sched_p;
	mFirstconstruct = false;
...
#ifdef  USE_SINGLE_THREAD_FOR_SENDER		
	case  kWhatScheduleSend:
	{
#ifdef MTB_SUPPORT		
        ATRACE_BEGIN("MediaSender, kWhatScheduleSend");
#endif
		if (mMode == MODE_TRANSPORT_STREAM) {
			 status_t err  = onScheduleSend();
			 if (err != OK) {
			    notifyError(err);	
			 }
...
status_t MediaSender::packetizeAccessUnit(
    ...
    if (mHDCP != NULL && !info.mIsAudio) {
	  ...
	  int64_t startUs  = ALooper::GetNowUs();	
        ...
        if (err != OK) {
            ALOGE("Failed to HDCP-encrypt media data (err %d)",err);
	...
	if(!accessUnit->meta()->findInt32("dummy-nal", &dummy)){
		  ...
		  int64_t timeStamp;
	          CHECK(accessUnit->meta()->findInt64("timeUs", &timeStamp));
		  sp<WfdDebugInfo> debugInfo= defaultWfdDebugInfo();
		  debugInfo->addTimeInfoByKey(!info.mIsAudio , timeStamp, "HdCp", (endUs-startUs)/1000);
	}
#endif
