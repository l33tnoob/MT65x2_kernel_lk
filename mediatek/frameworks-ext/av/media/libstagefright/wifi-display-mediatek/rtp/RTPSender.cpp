...
status_t RTPSender::queueTSPackets(
    ...
    CHECK(tsPackets->meta()->findInt64("timeUs", &timeUs));
     ...
     bool isVideo = false;
    int32_t dummy;
    if (tsPackets->meta()->findInt32("isVideo", &dummy) && dummy == 1) {
        isVideo = true;
	 mVideoCount ++;
    }
	...
	if(isVideo)
	{
		...
		int32_t dummy;
		if(!tsPackets->meta()->findInt32("dummy-nal", &dummy)){
			mCountFrames ++;
	...
	ALOGI("[WFD_P][%s] timestamp=%lld ms,size=%-6d,[Fps]=%d,[netTimeUs]=%-4lld ms,[delayUs]=%-6lld ms ,[Latency]=%-6lld ms",
			isVideo ? "video" : "audio",timeUs/1000,tsPackets->size(),sendFps,netTimeUs/1000,delayUs/1000,latencyTimeMs);
