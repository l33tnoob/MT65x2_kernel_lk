...
#ifdef __BTMTK__
    ALOGI("[JNI][API] create_prop_array : name=%s, type=%c, len=%d", property->name, property->type, len);
#endif
...
{
    ALOGI("[GAP] btmtk_util_print_uuid128 : %02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%02x", 
...
{
    ALOGI("pntohs(0x%02X%02X) : addr=%p", arr[0], arr[1], arr);
    if( (((U32)arr) & 0x1) )
    {
        unsigned short tmp;
        ALOGI("pntohs : use tmp");
...
{
    ALOGI("pntohl(0x%02X%02X%02X%02X) : addr=%p", arr[0], arr[1], arr[2], arr[3], arr);
    if( (((U32)arr) & 0x3) )
    {
        unsigned long tmp;
        ALOGI("pntohl : use tmp");
...
{
    ALOGI("phtons(0x%02X%02X) : addr=%p", arr[0], arr[1], arr);
    if( (((U32)arr) & 0x1) )
    {
        unsigned short tmp;
        ALOGI("phtons : use tmp");
        memcpy(&tmp, arr, sizeof(unsigned short));
        return htons(tmp);
    }
    else
...
{
    ALOGI("phtonl(0x%02X%02X%02X%02X) : addr=%p", arr[0], arr[1], arr[2], arr[3], arr);
    if( (((U32)arr) & 0x3) )
    {
        unsigned long tmp;
        ALOGI("phtonl : use tmp");
        memcpy(&tmp, arr, sizeof(unsigned long));
        return htonl(tmp);
    }
    else
...
{
    //unsigned short *data;
    ALOGI("[GAP] +btmtk_util_convert_uuid16_2_uuid128(0x%x)", uuid16);
...
void btmtk_util_convert_uuid128_2_string(char *buf, char *uuid128)
{
    ...
    ALOGI("[GAP] btmtk_util_convert_uuid128_2_string : %s", buf);
}
...
void btmtk_util_convert_juuid_2_uuid128(char *buf, long long msb, long long lsb)
{
    char tmp[16];
    int i;
    ALOGI("[GAP] +btmtk_util_convert_juuid_2_uuid128(%llx, %llx)", msb, lsb);
    memcpy(tmp, &lsb, 8);
    memcpy(tmp + 8, &msb, 8);
    for (i = 0; i < 16; i++)
    {
    ...
    ALOGI("[GAP] -btmtk_util_convert_juuid_2_uuid128 : %p", buf);
}
...
btmtk_sdp_req_struct *btmtk_util_find_sdp_channel_request(event_loop_native_data_t *nat, bt_addr_struct *addr, unsigned short uuid, unsigned short attrid)
{
    ...
    ALOGI("[GAP] btmtk_util_find_sdp_channel_request search 0x%lX:0x%X:0x%X: uudi=0x%X", addr->lap, addr->uap, addr->nap, uuid);
    while (ptr)
    {
        ALOGI("[GAP] btmtk_util_find_sdp_channel_request found 0x%lX:0x%X:0x%X", ptr->addr.lap, ptr->addr.uap, ptr->addr.nap);
