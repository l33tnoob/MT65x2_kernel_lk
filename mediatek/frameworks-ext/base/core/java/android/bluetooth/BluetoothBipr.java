...
public class BluetoothBipr implements BluetoothProfileManager.BluetoothProfileBehavior{
    
    private static final boolean DBG = true;
    private static final boolean VDBG = true;

    private static final String TAG = "BIP fw";

    private IBluetoothBipr mService;
    private final Context mContext;

    private ServiceConnection mConnection = new ServiceConnection() {
        public void onServiceConnected(ComponentName className, IBinder service) {
            mService = IBluetoothBipr.Stub.asInterface(service);
        }
    ...
    public BluetoothBipr (Context context) {
	mContext = context;
        if (!context.bindService(
		new Intent(IBluetoothBipr.class.getName()), mConnection, Context.BIND_AUTO_CREATE)) {
            Log.e(TAG, "Could not bind to Bluetooth BIP Service");
        }
    }
