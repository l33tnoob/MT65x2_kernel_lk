...
public class BluetoothBpp implements BluetoothProfileManager.BluetoothProfileBehavior{
    
    private static final boolean DBG = true;
    private static final boolean VDBG = true;

    private static final String TAG = "BPP fw";

    private IBluetoothBpp mService;
    private final Context mContext;

    private ServiceConnection mConnection = new ServiceConnection() {
        public void onServiceConnected(ComponentName className, IBinder service) {
            mService = IBluetoothBpp.Stub.asInterface(service);
        }
    ...
    public BluetoothBpp(Context context) {
	mContext = context;
        if (!context.bindService(
		new Intent(IBluetoothBpp.class.getName()), mConnection, Context.BIND_AUTO_CREATE)) {
