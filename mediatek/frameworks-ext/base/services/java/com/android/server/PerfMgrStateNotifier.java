...
public final class PerfMgrStateNotifier implements IActivityStateNotifier{
        ...
        */
        public void notifyActivityState(String packageName, String className, IActivityStateNotifier.ActivityState actState) {
            int state;
            //Slog.i(TAG, "[notifyActivityState] " + packageName + ", " + className + ", " + actState);

            switch(actState) {
            case Paused:
                state = PerfService.STATE_PAUSED;
                break;
            case Resumed:
                state = PerfService.STATE_RESUMED;
                break;
            case Destroyed:
                state = PerfService.STATE_DESTROYED;
                break;
            default:
                return;
            }
            PerfService.nativePerfNotifyAppState(packageName, className, state);
        }
        ...
        */
        public void notifyAppDied(HashSet<String> packageList)
        {
            ...
            while(i.hasNext()){
                ...
                PerfService.nativePerfNotifyAppState(packageName, null, PerfService.STATE_DEAD);
            }
        }
