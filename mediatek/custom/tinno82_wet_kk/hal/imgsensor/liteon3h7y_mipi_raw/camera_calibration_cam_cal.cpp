...
const CALIBRATION_LAYOUT_STRUCT CalLayoutTbl[MAX_CALIBRATION_LAYOUT_NUM]=
{
	...
	{//CALIBRATION_LAYOUT_SENSOR_OTP
	//head address    ID[module]
		0x00000A08, 0x00000003, CAM_CAL_SINGLE_OTP_DATA,
		{		
			{0x00000001, 0x00000000, 0x00000000, DoCamCalModuleVersion}, //CAMERA_CAM_CAL_DATA_MODULE_VERSION
			...
			//{0x0000000B, 0x00000005, 0x0000000E, DoCamCal2AGain}, //CAMERA_CAM_CAL_DATA_3A_GAIN
			{0x00000001, 0x00000005, 0x0000000E, DoCamCal2AGain}, //CAMERA_CAM_CAL_DATA_3A_GAIN
	...
	*/
    ...
    { 
        ...
        else
        {
            ...
            pCamCalData->Single2A.S2aAfBitflagEn = (0x0F & AWBAFConfig);// //Bit: step 0(inf.), 1(marco), 2, 3, 4,5,6,7 0x0c-->0x0f changed by xuezhen0326
            //memset(pCamCalData->Single2A.S2aAf,0x0,sizeof(pCamCalData->Single2A.S2aAf));
		...
		//if(0x2&AWBAFConfig){ //hcy modify flag: mark for read af data directly
		
		{
                ////AF////
                //cam_calCfg.u4Offset = (start_addr+14); //af address hcy
                cam_calCfg.u4Offset = (0x0A12); //af address hcy

                cam_calCfg.u4Length = 2;
                cam_calCfg.pu1Params = (u8 *)&AFInf;   //hcy         
                ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);	
                if(!ioctlerr)
                {                
    				err = CAM_CAL_ERR_NO_ERR;
                ...
                cam_calCfg.u4Offset = (0x0A10); //af address hcy
                cam_calCfg.u4Length = 2;
                cam_calCfg.pu1Params = (u8 *)&AFMacro;    //hcy        	
                ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);	
                if(!ioctlerr)
                {                
    				err = CAM_CAL_ERR_NO_ERR;
                ...
                pCamCalData->Single2A.S2aAf[0] = AFInf;// module hcy
                pCamCalData->Single2A.S2aAf[1] = AFMacro;//module hcy
    ...
    if ((gIsInitedCamCal==CAM_CAL_LAYOUT_RTN_QUEUE) && (CAM_CALInit() != CAM_CAL_NONE) && (CAM_CALDeviceName(&cBuf[0]) == 0))
    {
        ...
        for (i = 0; i< MAX_CALIBRATION_LAYOUT_NUM; i++)
        {
            if (cam_calCfg.u4Offset != CalLayoutTbl[i].HeaderAddr)
            {
                ...
                //err= ioctl(lCamcamFID, CAM_CALIOC_G_READ , &cam_calCfg);       	//mark for debug lsc
	        ...
	        CAM_CAL_LOG_IF(dumpEnable,"hcy Table[%d] ID= 0x%x  max=%d",i, CheckID,MAX_CALIBRATION_LAYOUT_NUM);
            if(CheckID == CalLayoutTbl[i].HeaderId)	//COMPARE MODULE Id 0x03 liteon
            {
                LayoutType = i;
