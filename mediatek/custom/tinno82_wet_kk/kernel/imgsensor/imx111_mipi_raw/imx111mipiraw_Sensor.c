...
#define IMX111MIPI_SENSOR_ID                    0x0111
...
#ifdef SUPPORT_IMX111_OTP  //LINE <> <DATE20130828> <imx111 otp> wupingzhou

#define SENSORDB_OTP(fmt, arg...) printk( "[IMX111MIPIRaw_OTP] "  fmt, ##arg)

// R/G and B/G of typical camera module is defined here
static int RG_Ratio_Typical = 0x24e;
static int BG_Ratio_Typical = 0x253;
...
static int update_wb(struct otp_struct *otp_info)
{
	...
	SENSORDB_OTP("IMX111_update_wb, 0x020F = 0x%x, 0x0211 = 0x%x, 0x0213 = 0x%x, 0x0215 = 0x%x\n", temp1,temp2,temp3,temp4);
	...
	SENSORDB_OTP("IMX111_update_wb, 0x020E = 0x%x, 0x0210 = 0x%x, 0x0212 = 0x%x, 0x0214 = 0x%x\n", temp1,temp2,temp3,temp4);
	...
	SENSORDB_OTP("IMX111_update_wb, 1 = 0x%x, 2 = 0x%x, 3 = 0x%x\n", R_test_H3,B_test_H3,G_test_H3);
...
static int check_otp_mid(struct otp_struct *otp_info)
{
	...
	{	
		...
		SENSORDB_OTP("IMX111_check_otp_mid, bank = %d, mid = 0x%x\n", bank, (*otp_info).module_integrator_id);
	...
	if ((i == 4)&&(((*otp_info).module_integrator_id) == 0))
		return 0;
	SENSORDB_OTP("IMX111_check_otp_mid success!\n");
	return 1;
}

static int check_otp_golden_wb(struct otp_struct *otp_info)
{
//¿ªÊ¼¼ì²â GOLDEN Sample AWB ratio
	int GoldenAddr[3] = {0x3520,0x3528,0x3530};
	int bank,i;
	for(i=0;i<3;i++)
	{
		bank = (GoldenAddr[i]&0xff)/8;
		IMX111MIPI_write_cmos_sensor(0x34c9, bank);
		(*otp_info).golden_rg_ratio = IMX111MIPI_read_cmos_sensor(GoldenAddr[i]);
		(*otp_info).golden_rg_ratio = (((*otp_info).golden_rg_ratio)<<8)+IMX111MIPI_read_cmos_sensor((GoldenAddr[i])+1);
		(*otp_info).golden_bg_ratio = IMX111MIPI_read_cmos_sensor((GoldenAddr[i])+2);
		(*otp_info).golden_bg_ratio = (((*otp_info).golden_bg_ratio)<<8)+IMX111MIPI_read_cmos_sensor((GoldenAddr[i])+3);
		(*otp_info).golden_GbGr_ratio = IMX111MIPI_read_cmos_sensor((GoldenAddr[i])+4);
		(*otp_info).golden_GbGr_ratio = (((*otp_info).golden_GbGr_ratio)<<8)+IMX111MIPI_read_cmos_sensor((GoldenAddr[i])+5);
		SENSORDB_OTP("IMX111_check_otp_golden_wb, bank:%d, golden_r/g:0x%x, golden_b/g:0x%x, golden_Gb/Gr:0x%x\n", bank, (*otp_info).golden_rg_ratio, (*otp_info).golden_bg_ratio, (*otp_info).golden_GbGr_ratio);
		if (((*otp_info).golden_rg_ratio) == 0)
			continue;
		else 
			break;
	}
	if ((i == 3)&&(((*otp_info).golden_rg_ratio) == 0))
		return 0;
	//OTP¿Õ¼äÒÑÉÕÂ¼GOLDEN Ratio
	SENSORDB_OTP("check_otp_golden_wb success!\n");
	return 1;
}

static int check_otp_wb_cs(struct otp_struct *otp_info)
{
	//¿ªÊ¼¼ì²â AWB & Color Shading
	int wbAddr[3] = {0x3538,0x3550,0x3568};
	int csAddr[3] = {0x353e,0x3556,0x356e};
	int bank,i,j;
	for(i=0;i<3;i++)
	{
		//WB
		...
		SENSORDB_OTP("IMX111_check_otp_wb_cs, bank:%d, r/g:0x%x, b/g:0x%x, Gb/Gr:0x%x\n", bank, (*otp_info).rg_ratio, (*otp_info).bg_ratio, (*otp_info).GbGr_ratio);
		//CS
		for(j=0;j<9;j++) {
			bank = ((csAddr[i]+j*2)&0xff)/8;
			IMX111MIPI_write_cmos_sensor(0x34c9, bank);
			(* otp_info).ROI_R[j]=IMX111MIPI_read_cmos_sensor((csAddr[i])+j*2);
			(* otp_info).ROI_B[j]=IMX111MIPI_read_cmos_sensor((csAddr[i])+j*2);
			SENSORDB_OTP("IMX111_check_otp_wb_cs, bank = %d, j = %d, ROI_R = 0x%x, ROI_B = 0x%x\n", bank, j, (* otp_info).ROI_R[j], (* otp_info).ROI_B[j]);
		}
	...
	SENSORDB_OTP("check_otp_wb_cs success!\n");
	return 1;
}
...
static int update_otp_info()
{
	...
	{
		SENSORDB_OTP("IMX111_check_otp_mid fail!\n");
		//¼ì²âMIDÊ§°Ü
		goto OTPERR;
	}
#if 0
	if (!(check_otp_golden_wb(&current_otp)))
	{
	
		SENSORDB_OTP("check_otp_golden_wb fail!\n");
		//¼ì²âGOLDEN RATIOÊ§°Ü
		goto OTPERR;
	}
#endif
	if (!(check_otp_wb_cs(&current_otp)))
	{
		SENSORDB_OTP("check_otp_wb_cs fail!\n");
...
//	IMX111MIPI_write_cmos_sensor(0x30C6, 0x00);// LINE <> <20131101> <otpimize the driver> panzaoyan
//	IMX111MIPI_write_cmos_sensor(0x30C7, 0x00);// LINE <> <20131101> <otpimize the driver> panzaoyan
	...
	IMX111MIPI_write_cmos_sensor(0x3301,0x04);	//LINE <> <20131101> <otpimize the driver> panzaoyan
...
void PreviewSetting(void)
	{	
		...
		IMX111MIPI_write_cmos_sensor(0x30A4,0x02);
		IMX111MIPI_write_cmos_sensor(0x303C,0x4B);
		mdelay(120);//LINE <> <20131101> <otpimize the driver> panzaoyan
		//size setting  s
   ...
   int  retry = 3; 
    ...
    do {		
	   *sensorID =(kal_uint16)((IMX111MIPI_read_cmos_sensor(0x0000)<<8) | IMX111MIPI_read_cmos_sensor(0x0001)); 
		...
		//BEGIN <> <DATE20130228> <switch camera problem> wupingzhou	
        	{
           		 mDELAY(60);//LINE <> <DATE20130228> <switch camera problem> wupingzhou
           		 break;
        	}
		//END <> <DATE20130228> <switch camera problem> wupingzhou
        SENSORDB("Read Sensor ID Fail = 0x%04x\n", *sensorID); 
        retry--; 
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	//	IMX111MIPI_SetShutter(IMX111MIPI_sensor.pv_shutter);
       ...
       IMX111MIPISetFlipMirror(IMAGE_HV_MIRROR);  //added by panzaoyan, 20130717
	return ERROR_NONE;
}	/* IMX111MIPIPreview() */
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	//	image_window->ExposureWindowHeight= IMX111MIPI_IMAGE_SENSOR_PV_HEIGHT - 2*iStartY;
		...
		IMX111MIPISetFlipMirror(IMAGE_HV_MIRROR);  //added by panzaoyan, 20130808
	return ERROR_NONE;
}	/* IMX111MIPIPreview() */
...
//    pSensorInfo->SensorOutputDataFormat=SENSOR_OUTPUT_FORMAT_RAW_B;
    pSensorInfo->SensorOutputDataFormat=SENSOR_OUTPUT_FORMAT_RAW_R;//LINE <> <DATE20130830> <imx111 colour problem> wupingzhou
    ...
    pSensorInfo->PreviewDelayFrame = 8; //LINE <SEFOS-412> <DATE20130924> <anti-flicker 60Hz,imx111 camera record,screen show pink.> wupingzhou
