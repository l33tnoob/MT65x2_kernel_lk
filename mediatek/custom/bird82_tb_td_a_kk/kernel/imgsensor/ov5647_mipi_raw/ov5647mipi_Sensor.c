...
static void OV5647MIPI_Sensor_1080P(void)
{
	...
	OV5647MIPI_write_cmos_sensor(0x3821,0x06); // mirror
	OV5647MIPI_write_cmos_sensor(0x3820,0x00); // no flip
...
static void OV5647MIPI_Sensor_1M(void)
{
	...
	OV5647MIPI_write_cmos_sensor(0x3821,0x06); // mirror
    OV5647MIPI_write_cmos_sensor(0x3820,0x41); // no flip
...
#define OV5647MIPI_TEST_PATTERN_CHECKSUM     (0xa43d5072)
UINT32 OV5647MIPISetTestPatternMode(kal_bool bEnable)
{
    SENSORDB("[OV5647MIPISetTestPatternMode] Test pattern enable:%d\n", bEnable);

    if(bEnable) 
    {
        OV5647MIPI_write_cmos_sensor(0x503D,0x80);      
    }
    else    
    {
        OV5647MIPI_write_cmos_sensor(0x503D,0x00);      
    }
    return ERROR_NONE;
}
                                ...
                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    SENSORDB("[OV5647][OV5647MIPIFeatureControl]feature id=%d \n",FeatureId);
    ...
    case SENSOR_FEATURE_SET_TEST_PATTERN:
        //1 TODO
        //SENSORDB("[OV5647MIPI] F_SET_TEST_PATTERN: FAIL: NOT Support\n");
        OV5647MIPISetTestPatternMode((BOOL)*pFeatureData16);
        break;

    case SENSOR_FEATURE_GET_TEST_PATTERN_CHECKSUM_VALUE://for factory mode auto testing
        *pFeatureReturnPara32= OV5647MIPI_TEST_PATTERN_CHECKSUM;
        *pFeatureParaLen=4;
        break;
