...
public class WorldPhoneWrapper implements IWorldPhone {
    private static int sOperator = -1;
    private static WorldPhoneOm sWorldPhoneOm = null;
    private static WorldPhoneOp01 sWorldPhoneOp01 = null;

    public WorldPhoneWrapper(int op, Context context, Phone phone) {
        sOperator = op;
        if (sOperator == IWorldPhone.POLICY_OM) {
            sWorldPhoneOm = new WorldPhoneOm(sOperator, context, phone);
        } else if (sOperator == IWorldPhone.POLICY_OP01) {
            sWorldPhoneOp01 = new WorldPhoneOp01(sOperator, context, phone);
        } else {
            logd("Invalid Operator:" + sOperator);
        }
    }
