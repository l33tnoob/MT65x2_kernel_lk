...
public class StaticLayoutDirectionsTest extends TestCase {
    ...
    private static Directions dirs(int ... dirs) {
        return TextUtils.newDirectionsForTest(dirs);
    }
	...
	private static final Directions DIRS_ALL_LEFT_TO_RIGHT = TextUtils.newDirectionsForTest(new int[] { 0, RUN_LENGTH_MASK }) ;        
    private static final Directions DIRS_ALL_RIGHT_TO_LEFT = TextUtils.newDirectionsForTest((new int[] { 0, RUN_LENGTH_MASK | RUN_RTL_FLAG })) ;        
    ...
    */
    ...
    private void checkDirections(Layout l, int i, String text,
        ...
        Directions result = l.getLineDirections(0);
        if (!Arrays.equals(TextUtils.getDirectionsForTest(expected), TextUtils.getDirectionsForTest(result))) {
            f.format("%n[%2d] '%s', %s != %s", i, text,
                    hexArray(TextUtils.getDirectionsForTest(expected)),
                    hexArray(TextUtils.getDirectionsForTest(result)));
        }
    }

    private void expectDirections(String msg, Directions expected, Directions result) {
        if (!Arrays.equals(TextUtils.getDirectionsForTest(expected), TextUtils.getDirectionsForTest(result))) {
            fail("expected: " + hexArray(TextUtils.getDirectionsForTest(expected)) +
                    " got: " + hexArray(TextUtils.getDirectionsForTest(result)));
        }
    }
}
