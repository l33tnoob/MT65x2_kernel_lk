...
public class DynamicLayoutBlocksTest extends TestCase {
    ...
    private void update(int startLine, int endLine, int newLineCount) {        
        TextUtils.setBlocksDataForTest(dl, initialBlockEnds, initialBlockIndices, initialBlockEnds.length);
        checkInvariants();        
        TextUtils.updateBlocksForTest(dl, startLine, endLine, newLineCount) ;
    }
    ...
    private void assertState(int[] sizes, int[] indices) {
        ...
        for (int i = 0; i < dl.getNumberOfBlocks(); i++) {
            ...
            /// M: temporarily remove and confirming later
