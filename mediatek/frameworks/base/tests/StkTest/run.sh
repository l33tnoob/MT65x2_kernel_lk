...
then
  ...
  # mediatek_packages_apps_Stk1	
  # STK	
  adb push $PRODUCT_OUT/system/app/Stk1.apk /system/app
  
  # mediatek_packages_apps_StkSelection	
  # STK	
  adb push $PRODUCT_OUT/system/app/Stk1.apk /system/app

  # install test APK
  adb uninstall com.mtk.sanitytest.launchstktest
  adb install -r $SRC_ROOT/___test_report/launchstktest.apk
  ...
  rm -rf $SRC_ROOT/___test_report/mtklog_stk
  ...
  TEST_PACKAGE=com.mtk.sanitytest.launchstktest
  ...
  3rm $SRC_ROOT/___test_report/junit-report-stk.xml
  #rm $SRC_ROOT/___test_report/junit-report-vt.xml
  adb pull /data/data/$PACKAGE/files/junit-report.xml $SRC_ROOT/___test_report/junit-report-stk.xml
       
  log_path=`cat $SRC_ROOT/.log_path`
  echo $log_path
  if [ "$log_path" = "persist.mtklog.log2sd.path = /mnt/sdcard2" ]; then              
    adb pull /sdcard2/mtklog $SRC_ROOT/___test_report/mtklog_stk  
  else
    adb pull /sdcard/mtklog $SRC_ROOT/___test_report/mtklog_stk    
  fi
