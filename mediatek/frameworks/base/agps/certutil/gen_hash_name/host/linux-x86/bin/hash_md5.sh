...
for cert in `ls CERT`
  ...
  then
     ...
     ${my_certutil} x509 -in CERT/${cert} -out 0_pem_md5/${hash_name}.0
     ${my_certutil} x509 -in CERT/${cert} -noout -text -fingerprint >> 0_pem_md5/${hash_name}.0
  else
     ...
     then
       echo hash_name=$hash_name
       ${my_certutil} x509 -in CERT/${cert} -inform DER -out 0_pem_md5/${hash_name}.0
       ${my_certutil} x509 -in CERT/${cert} -inform DER -noout -text -fingerprint >> 0_pem_md5/${hash_name}.0
     else
       ...
       then
         echo hash_name=$hash_name
         ${my_certutil} x509 -in CERT/${cert} -inform PEM -out 0_pem_md5/${hash_name}.0
         ${my_certutil} x509 -in CERT/${cert} -inform PEM -noout -text -fingerprint >> 0_pem_md5/${hash_name}.0
       fi
     fi
  fi
done
