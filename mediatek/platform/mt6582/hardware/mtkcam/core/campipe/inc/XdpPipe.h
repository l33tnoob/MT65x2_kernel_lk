 ...
 ************************************************************************************************/
#ifndef _MTK_CAMERA_CORE_CAMPIPE_INC_XDP_PIPE_H_
#define _MTK_CAMERA_CORE_CAMPIPE_INC_XDP_PIPE_H_
//
#include <vector>
//
using namespace std;
...
private:
    ...
    MBOOL mapDpFormat(EImageFormat fmt, MUINT32 h, const MUINT32 stride_p[3], 
         DpColorFormat& dp_fmt, MUINT32& planenum, MUINT32 stride[3], MUINT32 size[3]);

    MBOOL mapPhyAddr( BufInfo* buf );
    MBOOL unmapPhyAddr( BufInfo* buf );

    //imem
    IMemDrv* mIMemDrv;
    //MDP
    DpIspStream *mpStream;
    MUINT32 mu4SrcPlaneNumber;
    MUINT32 mu4DstPlaneNumber[MAX_NUM_OUTPORT_XDP];    

    struct PlaneSize{
        MUINT32 size[3];
    };
    struct PlaneSize mSrcPlaneSize;
    struct PlaneSize mDstPlaneSize[MAX_NUM_OUTPORT_XDP];

    BufInfo mSrcBuf;
    BufInfo mDstBuf[ MAX_NUM_OUTPORT_XDP ];
...
#endif  //  _MTK_CAMERA_CORE_CAMPIPE_INC_XDP_PIPE_H_
