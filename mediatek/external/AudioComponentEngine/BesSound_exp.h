...
struct BS_HandleStruct{
    ...
    */
    int (*GetBufferSize)(BS_Handle *p_handle,
                         unsigned int *p_internal_buf_size_in_byte,
    ...
    /*
        ...
        enable_OLA              [I] 0: Disable OLA for switching on / off BesSound engine
                                    1: Enable  OLA for switching on / off BesSound engine
    */
    int (*Open)( BS_Handle *p_handle,
                 char *p_internal_buf, 
                 const void *p_init_param,
                 int enable_OLA);
    
    /*
        Close this BesSound engine
        Return value     < 0  : Error 
                         >= 0 : Normal
        p_handle     [I] Handle of the current BesSound engine
        enable_OLA   [I] 0: Disable OLA for switching on / off BesSound engine
    ...
    */
    int (*SetParameters)(   BS_Handle *p_handle,
                            const void *p_runtime_param);
    
    /*
        Get the information about this BesSound engine
        Return value      < 0  : Error 
                          >= 0 : Normal
        p_handle      [I] Handle of the current BesSound engine
        p_engine_info [O] Pointer to the structure of the engine information
    */
    int (*GetEngineInfo)(   BS_Handle *p_handle,
                            BS_EngineInfo *p_engine_info);
    ...
    */
    int (*Process)( BS_Handle *p_handle,
                    char *p_temp_buf,
                    void *p_in_buf,
                    int  *p_in_byte_cnt,
                    void *p_ou_buf,
