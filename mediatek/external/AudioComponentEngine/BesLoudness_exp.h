...
typedef enum {
    ...
    FILT_MODE_AUD_ENH  = 4, // Enable BesAudEnh Algorithnm
} FILT_MODE_ENUM;
...
|                           for mono input 
|                           or both L / R channels 
|                           or L channel only (separate L / R filter)
|   pFilter_Coef_R          Pointer to the filter coefficient fields 
|                           for R channel only (separate L / R filter)
|
|**************************************************************************************/

// Only support 32-bit Q7.25 input, 32-bit Q7.25 output
typedef struct {
   ...
   BLOUD_FilterCoef  *pFilter_Coef_L;
   BLOUD_FilterCoef  *pFilter_Coef_R;
}BLOUD_InitParam;

int BLOUD_SetHandle(BS_Handle *p_handle);
