...
static  unsigned int get_crc32(unsigned int  crc_init_value, unsigned char * data_buffer, unsigned int data_size)
{
    unsigned int crc_value   = crc_init_value;

    while(data_size--) {
        crc_value = (crc_value >> 8) ^ crc32_table[(crc_value&0xFF) ^ *(data_buffer++)];
    }

    //return crc_value^crc_init_value;
    return crc_value;
}

int code_crc_handler(char* cmdline, ATOP_t at_op, char* response){
    ...
    if (access(SYSDEV, R_OK) < 0) {
        ALOGE("AT_CODE_CMD " "Permission denied: %s can't be open, errno = %d", SYSDEV,errno);
        sprintf(response,"\r\nERROR\r\n\r\n");
        return 0;
    }

    fd = open(SYSDEV, O_RDONLY);
    if (fd<0){
        ALOGE("AT_CODE_CMD " "Open %s failed, errno = %d", SYSDEV,errno);
        sprintf(response,"\r\nERROR\r\n\r\n");
        return 0;
    }

    if (stat(SYSDEV, &finfo) < 0) {
        ALOGE("AT_CODE_CMD " "Stat %s failed, errno = %d", SYSDEV,errno);
        sprintf(response,"\r\nERROR\r\n\r\n");
        return 0;
    }
    ...
    do {
        if (read(fd, &buff, 1) != 1) {
            ALOGE("AT_CODE_CMD " "Read %s failed, errno = %d", SYSDEV, errno);
            sprintf(response,"\r\nERROR\r\n\r\n");
            return 0;
        }

        crc = get_crc32(crc, &buff, 1);
    } while (++len < finfo.st_size);
    crc = crc ^ 0xffffffff;
    close(fd);
