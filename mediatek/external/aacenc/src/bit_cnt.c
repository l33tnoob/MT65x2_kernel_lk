  ...
  switch (codeBook) {
    ...
    case CODE_BOOK_1_NO:
      for(i=0; i<width; i+=4) {
        ...
        t0 = min(t0, 2);  t0 = max(t0, 0);
        t1 = min(t1, 2);  t1 = max(t1, 0);
        t2 = min(t2, 2);  t2 = max(t2, 0);
        t3 = min(t3, 2);  t3 = max(t3, 0);
        
        codeWord   = huff_ctab1[t0][t1][t2][t3];
        codeLength = HI_LTAB(huff_ltab1_2[t0][t1][t2][t3]);
    ...
    case CODE_BOOK_2_NO:
      for(i=0; i<width; i+=4) {
        ...
        codeWord   = huff_ctab2[t0][t1][t2][t3];
        codeLength = LO_LTAB(huff_ltab1_2[t0][t1][t2][t3]);
    ...
    case CODE_BOOK_5_NO:
      for(i=0; i<width; i+=2) {
        ...
        t0 = min(t0, 8);  t0 = max(t0, 0);
        t1 = min(t1, 8);  t1 = max(t1, 0);
        
        codeWord   = huff_ctab5[t0][t1];
        codeLength = HI_LTAB(huff_ltab5_6[t0][t1]);
#endif
        WriteBits(hBitstream,codeWord,codeLength);
      }
      break;
    ...
    case CODE_BOOK_6_NO:
      for(i=0; i<width; i+=2) {
        ...
        codeWord   = huff_ctab6[t0][t1];
        codeLength = LO_LTAB(huff_ltab5_6[t0][t1]);
#endif
