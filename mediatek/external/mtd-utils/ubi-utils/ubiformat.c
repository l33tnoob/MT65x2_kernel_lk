...
#ifdef MTK_COMBO_NAND_SUPPORT
		if((mtd->eb_size*eb)% (COMBO_NAND_BLOCK_SIZE) !=0) {
			printf(", COMBO NAND enabled, leave\n");
			continue;
		}
#endif
...
#ifdef MTK_COMBO_NAND_SUPPORT
	ubigen_info_init(&ui, COMBO_NAND_BLOCK_SIZE, COMBO_NAND_PAGE_SIZE, COMBO_NAND_PAGE_SIZE,
			 COMBO_NAND_PAGE_SIZE, args.ubi_ver, args.image_seq);
#else
