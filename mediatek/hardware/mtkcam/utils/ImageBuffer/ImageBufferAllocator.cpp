...
)
{
    ...
    pImgBuf = alloc_ion(szCallerName, rImgParam);
#else
// TBD
#endif
    //
...
#if defined(MTK_ION_SUPPORT)
    //
    IonImageBufferHeap::AllocExtraParam extraParam;
    sp<IImageBufferHeap> pHeap = IonImageBufferHeap::create(szCallerName, rImgParam, extraParam);
    if  ( pHeap == 0 ) {
        CAM_LOGE("NULL Heap\n");
        return NULL;
    }
