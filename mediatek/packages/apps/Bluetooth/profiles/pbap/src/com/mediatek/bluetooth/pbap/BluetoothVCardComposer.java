...
public class BluetoothVCardComposer {
    ...
    static {
        Uri.Builder builder = RawContactsEntity.CONTENT_URI.buildUpon();
        builder.appendQueryParameter(RawContactsEntity.FOR_EXPORT_ONLY, "1");
    ...
    public interface OneEntryHandler {
        ...
        boolean onEntryCreated(String vcard);
            ...
            if (mWriter != null) {
                try {
                    ...
                    try {
                        ...
                        Log.d(LOG_TAG, "onTerminate," + e.getMessage());
                    }
                }
            }
        }
...
//        }
    ...
    // TODO: ues extra MIME-TYPE instead of adding this kind of inflexible
    // fields
    ...
    private static final Map<Integer, String> IM_MAP;

    static {
        IM_MAP = new HashMap<Integer, String>();
        IM_MAP.put(Im.PROTOCOL_AIM, Constants.PROPERTY_X_AIM);
        IM_MAP.put(Im.PROTOCOL_MSN, Constants.PROPERTY_X_MSN);
        IM_MAP.put(Im.PROTOCOL_YAHOO, Constants.PROPERTY_X_YAHOO);
        IM_MAP.put(Im.PROTOCOL_ICQ, Constants.PROPERTY_X_ICQ);
        IM_MAP.put(Im.PROTOCOL_JABBER, Constants.PROPERTY_X_JABBER);
        IM_MAP.put(Im.PROTOCOL_SKYPE, Constants.PROPERTY_X_SKYPE_USERNAME);
    ...
    private boolean mUsePbapNameImprove = true;
    ...
    private static final String[] CALL_LOG_PROJECTION = new String[] {
            Calls.NUMBER, Calls.DATE, Calls.TYPE, Calls.CACHED_NAME, Calls.CACHED_NUMBER_TYPE, Calls.CACHED_NUMBER_LABEL
    };
    ...
    public BluetoothVCardComposer(Context context, String vcardTypeStr, boolean careHandlerErrors) {
        this(context, VCardConfig.getVCardTypeFromString(vcardTypeStr), careHandlerErrors, false, true);
    }

    public BluetoothVCardComposer(Context context, int vcardType, boolean careHandlerErrors) {
     ...
     */
    public BluetoothVCardComposer(Context context, int vcardType, boolean careHandlerErrors, boolean isCallLogComposer,
            boolean needPhotoInVCard) {
        mContext = context;
    ...
    public String composeVCardForPhoneOwnNumber(int phonetype, String phoneName, String phoneNumber, boolean vcardVer21) {
        ...
        // TODO: QP should be used? Using mUsesQPToPrimaryProperties should
        // help.
    ...
    public boolean init() {
        return init(null, null, null/* SH modified */);
    }

    /**
     * @return Returns true when initialization is successful and all the other methods are available. Returns false
     *         otherwise.
     */
    public boolean init(final String selection, final String[] selectionArgs, final String orderBy) { // SH modified
        ...
        Log.d(LOG_TAG, "mIsCallLogComposer=" + String.valueOf(mIsCallLogComposer)); // SH Log
        Log.d(LOG_TAG, "init(" + selection + "," + selectionArgs + "," + orderBy + ")"); // SH Log
        if (mIsCallLogComposer) {
            mCursor = mContentResolver.query(CallLog.Calls.CONTENT_URI, CALL_LOG_PROJECTION, selection, selectionArgs,
                    orderBy); // SH modified
        } else {
            mCursor = mContentResolver.query(Contacts.CONTENT_URI, CONTACTS_PROJECTION, 
                    selection, selectionArgs, orderBy); // SH
            // modified
        }

        if (mCursor == null) {
            Log.d(LOG_TAG, "mCursor == null"); // SH Log
            mErrorReason = FAILURE_REASON_FAILED_TO_GET_DATABASE_INFO;
            return false;
        }

        if (getCount() == 0 || !mCursor.moveToFirst()) {
            Log.d(LOG_TAG, "getCount()=" + getCount()); // SH Log
            try {
                mCursor.close();
    ...
    /**
     * Format according to RFC 2445 DATETIME type. The format is: ("%Y%m%dT%H%M%SZ").
     */
    ...
    /**
     * Try to append the property line for a call history time stamp field if possible. Do nothing if the call log type
     * gotton from the database is invalid.
     */
    ...
    private String createOneEntryInternal(final String contactId) {
        ...
        try {
            ...
            Cursor c = mContentResolver.query(DATA_REQUEST_URI, null, selection, selectionArgs, null);
            if (c != null) {
                entityIterator = RawContacts.newEntityIterator(c);
        ...
        /* SH Modified */
        if ((mFilter & VCARD_ATTRIB_NICKNAME) != 0) {
            Log.d(LOG_TAG, "appendNickName");
            appendNickNames(builder, contentValuesListMap);
        }
        appendPhones(builder, contentValuesListMap);
        /* SH Modified */
        if ((mFilter & VCARD_ATTRIB_EMAIL) != 0) {
            Log.d(LOG_TAG, "appendEmails");
            appendEmails(builder, contentValuesListMap);
        }
        /* SH Modified */
        if ((mFilter & VCARD_ATTRIB_ADR) != 0) {
            Log.d(LOG_TAG, "appendPostals");
        ...
        // }
        /* SH modified */
        if ((mFilter & VCARD_ATTRIB_URL) != 0) {
            appendWebsites(builder, contentValuesListMap);
        }
        /* SH Modified */
        if ((mFilter & VCARD_ATTRIB_BDAY) != 0) {
            appendBirthday(builder, contentValuesListMap);
        }
        /* SH modified */
        if ((mFilter & VCARD_ATTRIB_ORG) != 0) {
            appendOrganizations(builder, contentValuesListMap);
        }
        /* SH modified */
        if ((mFilter & VCARD_ATTRIB_TITLE) != 0) {
            appendTitles(builder, contentValuesListMap);
        }
        /* SH Modified */
        if (mNeedPhotoForVCard && (mFilter & VCARD_ATTRIB_PHOTO) != 0) {
            appendPhotos(builder, contentValuesListMap);
        }
        /* SH Modified */
        if ((mFilter & VCARD_ATTRIB_NOTE) != 0) {
...
//    }
    ...
    private void appendStructuredNames(final StringBuilder builder,
        ...
        if (contentValuesList != null && contentValuesList.size() > 0) {
            ...
            // In PBAP, vCard 2.1 requires "N" properties.
            appendVCardLine(builder, VCARD_PROPERTY_NAME, "");
        }
        /*************/
    }
    ...
    private boolean containsNonEmptyName(ContentValues contentValues) {
        ...
        return !(TextUtils.isEmpty(familyName) && TextUtils.isEmpty(middleName) && TextUtils.isEmpty(givenName)
                && TextUtils.isEmpty(prefix) && TextUtils.isEmpty(suffix) && TextUtils.isEmpty(displayName));
    }

    private void appendStructuredNamesInternal(final StringBuilder builder, final List<ContentValues> contentValuesList) {
        // For safety, we'll emit just one value around StructuredName, as
        // external importers
        // may get confused with multiple "N", "FN", etc. properties, though it
        // is valid in
        // vCard spec.
        ...
        if (!TextUtils.isEmpty(familyName) || !TextUtils.isEmpty(givenName)) {
            ...
            final boolean reallyUseQuotedPrintableToName = (mUsesQPToPrimaryProperties && !(VCardUtils
                    ...
                    && VCardUtils.containsOnlyNonCrLfPrintableAscii(prefix) && VCardUtils
                    .containsOnlyNonCrLfPrintableAscii(suffix)));
            ...
            // N property. This order is specified by vCard spec and does not
            // depend on countries.
            
            // Improve: For PBAP scenario, provide display name for remote device to show. It is more friendly. 
            // Because some device will parse the name as an unexpected result. E.g. some device will not parse the "Middle Name"
            if ( mUsePbapNameImprove && !TextUtils.isEmpty(displayName)) {
                final boolean reallyUseQuotedPrintableToDisplayName = (mUsesQPToPrimaryProperties && !VCardUtils
                    .containsOnlyNonCrLfPrintableAscii(displayName));
                final String encodedDisplayName = reallyUseQuotedPrintableToDisplayName ? encodeQuotedPrintable(displayName)
            ...
            final String fullname = VCardUtils.constructNameFromElements(VCardConfig.getNameOrderType(mVCardType),
                    encodedFamily, encodedMiddle, encodedGiven, encodedPrefix, encodedSuffix);
            final boolean reallyUseQuotedPrintableToFullname = mUsesQPToPrimaryProperties
                    && !VCardUtils.containsOnlyNonCrLfPrintableAscii(fullname);

            final String encodedFullname = reallyUseQuotedPrintableToFullname ? encodeQuotedPrintable(fullname)
            ...
            // vCard 3.0 should add full name
            if (mIsV30) {
                builder.append(VCARD_PROPERTY_FULL_NAME);
        ...
        if (!(TextUtils.isEmpty(phoneticFamilyName) && TextUtils.isEmpty(phoneticMiddleName) && TextUtils
            ...
            if (mIsV30) {
                ...
                // Do not need to care about QP, since vCard 3.0 does not allow
                // it.
                ...
                boolean reallyUseQuotedPrintable = (mUsesQPToPrimaryProperties && !(VCardUtils
                        .containsOnlyNonCrLfPrintableAscii(phoneticFamilyName)
                        && VCardUtils.containsOnlyNonCrLfPrintableAscii(phoneticMiddleName) && VCardUtils
                        .containsOnlyNonCrLfPrintableAscii(phoneticGivenName)));
                ...
                if (shouldAppendCharsetAttribute(Arrays.asList(encodedPhoneticFamilyName, encodedPhoneticMiddleName,
        ...
        if (mUsesDefactProperty) {
            if (!TextUtils.isEmpty(phoneticGivenName)) {
                final boolean reallyUseQuotedPrintable = (mUsesQPToPrimaryProperties && !VCardUtils
                        .containsOnlyNonCrLfPrintableAscii(phoneticGivenName));
                final String encodedPhoneticGivenName;
                if (reallyUseQuotedPrintable) {
                    encodedPhoneticGivenName = encodeQuotedPrintable(phoneticGivenName);
            ...
            if (!TextUtils.isEmpty(phoneticMiddleName)) {
                final boolean reallyUseQuotedPrintable = (mUsesQPToPrimaryProperties && !VCardUtils
                        .containsOnlyNonCrLfPrintableAscii(phoneticMiddleName));
                final String encodedPhoneticMiddleName;
                if (reallyUseQuotedPrintable) {
                    encodedPhoneticMiddleName = encodeQuotedPrintable(phoneticMiddleName);
                } else {
    ...
    private void appendNickNames(final StringBuilder builder, final Map<String, List<ContentValues>> contentValuesListMap) {
        ...
        if (contentValuesList != null) {
            ...
            for (ContentValues contentValues : contentValuesList) {
                ...
                final boolean reallyUseQuotedPrintable = (mUsesQuotedPrintable && !VCardUtils
                        .containsOnlyNonCrLfPrintableAscii(nickname));
                if (reallyUseQuotedPrintable) {
    ...
    private void appendPhones(final StringBuilder builder, final Map<String, List<ContentValues>> contentValuesListMap) {
                ...
                if (type == Phone.TYPE_PAGER) {
                    ...
                    for (String actualPhoneNumber : phoneNumberList) {
                        if (!phoneSet.contains(actualPhoneNumber)) {
                            ...
                            SpannableStringBuilder tmpBuilder = new SpannableStringBuilder(actualPhoneNumber);
    ...
    private void appendEmails(final StringBuilder builder, final Map<String, List<ContentValues>> contentValuesListMap) {
    ...
    private void appendPostals(final StringBuilder builder, final Map<String, List<ContentValues>> contentValuesListMap) {
        final List<ContentValues> contentValuesList = contentValuesListMap.get(StructuredPostal.CONTENT_ITEM_TYPE);
        if (contentValuesList != null) {
            if (mIsDoCoMo) {
     ...
     */
    private void appendPostalsForDoCoMo(final StringBuilder builder, final List<ContentValues> contentValuesList) {
        // TODO: from old, inefficient code. fix this.
        if (appendPostalsForDoCoMoInternal(builder, contentValuesList, StructuredPostal.TYPE_HOME)) {
            return;
        }
        if (appendPostalsForDoCoMoInternal(builder, contentValuesList, StructuredPostal.TYPE_WORK)) {
            return;
        }
        if (appendPostalsForDoCoMoInternal(builder, contentValuesList, StructuredPostal.TYPE_OTHER)) {
            return;
        }
        if (appendPostalsForDoCoMoInternal(builder, contentValuesList, StructuredPostal.TYPE_CUSTOM)) {
            return;
        }
    ...
    private boolean appendPostalsForDoCoMoInternal(final StringBuilder builder, final List<ContentValues> contentValuesList,
            Integer preferedType) {
        for (ContentValues contentValues : contentValuesList) {
    ...
    private void appendPostalsForGeneric(final StringBuilder builder, final List<ContentValues> contentValuesList) {
        for (ContentValues contentValues : contentValuesList) {
            final Integer type = contentValues.getAsInteger(StructuredPostal.TYPE);
            final String label = contentValues.getAsString(StructuredPostal.LABEL);
            if (type != null) {
    ...
    private void appendIms(final StringBuilder builder, final Map<String, List<ContentValues>> contentValuesListMap) {
    ...
    private void appendWebsites(final StringBuilder builder, final Map<String, List<ContentValues>> contentValuesListMap) {
        final List<ContentValues> contentValuesList = contentValuesListMap.get(Website.CONTENT_ITEM_TYPE);
        if (contentValuesList != null) {
            for (ContentValues contentValues : contentValuesList) {
    ...
    private void appendBirthday(final StringBuilder builder, final Map<String, List<ContentValues>> contentValuesListMap) {
        ...
        if (contentValuesList != null && contentValuesList.size() > 0) {
            ...
            // Theoretically, there must be only one birthday for each vCard
            // data and
    ...
    private void appendOrganizations(final StringBuilder builder, final Map<String, 
            List<ContentValues>> contentValuesListMap) {
        final List<ContentValues> contentValuesList = contentValuesListMap.get(Organization.CONTENT_ITEM_TYPE);
        if (contentValuesList != null) {
            for (ContentValues contentValues : contentValuesList) {
                ...
                /* SH Modified */
                /*
                 * String title = contentValues .getAsString(Organization.TITLE); if (title != null) { title = title.trim();
                 * }
                 */
                /*************/

                if (company != null && !TextUtils.isEmpty(company)) {
                    appendVCardLine(builder, VCARD_PROPERTY_ORG, company, !VCardUtils.containsOnlyPrintableAscii(company),
                            (mUsesQuotedPrintable && !VCardUtils.containsOnlyNonCrLfPrintableAscii(company)));
                }
                /* SH Modified */
                /*
                 * if (!TextUtils.isEmpty(title)) { appendVCardLine(builder, VCARD_PROPERTY_TITLE, title,
                 * !VCardUtils.containsOnlyPrintableAscii(title), (mUsesQuotedPrintable &&
    ...
    private void appendPhotos(final StringBuilder builder, final Map<String, List<ContentValues>> contentValuesListMap) {
        ...
        if (contentValuesList != null) {
            for (ContentValues contentValues : contentValuesList) {
                ...
                // TODO: there should be some general API for detecting the file
                // format.
                if (data.length >= 3 && data[0] == 'G' && data[1] == 'I' && data[2] == 'F') {
                    photoType = "GIF";
                } else if (data.length >= 4 && data[0] == (byte) 0x89 && data[1] == 'P'
                        && data[2] == 'N' && data[3] == 'G') {
    ...
    private void appendNotes(final StringBuilder builder, final Map<String, List<ContentValues>> contentValuesListMap) {
        final List<ContentValues> contentValuesList = contentValuesListMap.get(Note.CONTENT_ITEM_TYPE);
        if (contentValuesList != null) {
            if (mOnlyOneNoteFieldIsAvailable) {
                ...
                appendVCardLine(builder, VCARD_PROPERTY_NOTE, noteStr, shouldAppendCharsetInfo, reallyUseQuotedPrintable);
            } else {
                for (ContentValues contentValues : contentValuesList) {
                    ...
                    if (noteStr != null && !TextUtils.isEmpty(noteStr)) {
                        ...
                        final boolean reallyUseQuotedPrintable = (mUsesQuotedPrintable && !VCardUtils
                                .containsOnlyNonCrLfPrintableAscii(noteStr));
                        appendVCardLine(builder, VCARD_PROPERTY_NOTE, noteStr, shouldAppendCharsetInfo,
    ...
    /**
     * Append '\' to the characters which should be escaped. The character set is different not only between vCard 2.1 and
     * vCard 3.0 but also among each device. Note that Quoted-Printable string must not be input here.
     */
    @SuppressWarnings("fallthrough")
    private String escapeCharacters(final String unescaped) {
        if (unescaped == null || TextUtils.isEmpty(unescaped)) {
            return "";
        }
        ...
        for (int i = 0; i < length; i++) {
            ...
            switch (ch) {
                ...
                case '\r':
                    if (i + 1 < length) {
                        char nextChar = unescaped.charAt(i + 1);
                        if (nextChar == '\n') {
                            continue;
                        }
                        // else {
...
//                    }
    ...
    private void appendVCardPhotoLine(final StringBuilder builder, final String encodedData, final String photoType) {
    ...
    private void appendVCardPostalLine(final StringBuilder builder, final Integer typeAsObject, final String label,
            final ContentValues contentValues) {
        builder.append(VCARD_PROPERTY_ADR);
        builder.append(VCARD_ATTR_SEPARATOR);

        // Note: Not sure why we need to emit "empty" line even when actual data
        // does not exist.
        ...
        switch (typeAsPrimitive) {
            ...
            case StructuredPostal.TYPE_CUSTOM:
                if (mUsesAndroidProperty && label != null && !TextUtils.isEmpty(label)
        ...
        if (dataExists) {
            if (shouldAppendCharset) {
                // Strictly, vCard 3.0 does not allow exporters to emit
                // charset information,
                // but we will add it since the information should be useful
                // for importers,
                //
                // Assume no parser does not emit error with this attribute
                // in vCard 3.0.
    ...
    private void appendVCardEmailLine(final StringBuilder builder, final Integer typeAsObject, final String label,
            final String data) {
        ...
        switch (typeAsPrimitive) {
             case Email.TYPE_CUSTOM:
                ...
                // Detail: Until Donut, there isn't TYPE_MOBILE for email while
                // there is now.
                // To support mobile type at that time, this custom label had
                // been used.
    ...
    private void appendVCardTelephoneLine(final StringBuilder builder, final Integer typeAsObject, final String label,
            String encodedData) {
        ...
        switch (typeAsPrimitive) {
            ...
            case Phone.TYPE_PAGER:
                if (mIsDoCoMo) {
                    ...
                    // Also, refrain from using appendType() so that "TYPE=" is
                    // never be appended.
    ...
    private void appendVCardLine(final StringBuilder builder, final String propertyName, final String rawData) {
        appendVCardLine(builder, propertyName, rawData, false, false);
    }

    private void appendVCardLine(final StringBuilder builder, final String field, final String rawData,
    ...
    private void appendTypeAttributes(final StringBuilder builder, final List<String> types) {
    ...
    /**
     * Returns true when the property line should contain charset attribute information. This method may return true even
     * when vCard version is 3.0. Strictly, adding charset information is invalid in VCard 3.0. However we'll add the info
     * only when used charset is not UTF-8 in vCard 3.0 format, since parser side may be able to use the charset via this
     * field, though we may encounter another problem by adding it... e.g. Japanese mobile phones use Shift_Jis while RFC
     * 2426 recommends UTF-8. By adding this field, parsers may be able to know this text is NOT UTF-8 but Shift_Jis.
     */
    ...
    /* FILTER map */
    public static final int VCARD_ATTRIB_VERSION = (1 << 0); /* vCard Version */

    public static final int VCARD_ATTRIB_FN = (1 << 1); /* Formatted Name */

    public static final int VCARD_ATTRIB_N = (1 << 2); /*
                                                        * Structured Presentation of Name
                                                        */

    public static final int VCARD_ATTRIB_PHOTO = (1 << 3); /*
                                                            * Associated Image or Photo
                                                            */

    public static final int VCARD_ATTRIB_BDAY = (1 << 4); /* Birthday */

    public static final int VCARD_ATTRIB_ADR = (1 << 5); /* Delivery Address */

    public static final int VCARD_ATTRIB_LABEL = (1 << 6); /* Delivery */

    public static final int VCARD_ATTRIB_TEL = (1 << 7); /* Delivery */

    public static final int VCARD_ATTRIB_EMAIL = (1 << 8); /*
                                                            * Electronic Mail Address
                                                            */

    public static final int VCARD_ATTRIB_MAILER = (1 << 9); /* Electronic Mail */

    public static final int VCARD_ATTRIB_TZ = (1 << 10); /* Time Zone */

    public static final int VCARD_ATTRIB_GEO = (1 << 11); /* Geographic Position */

    public static final int VCARD_ATTRIB_TITLE = (1 << 12); /* Job */

    public static final int VCARD_ATTRIB_ROLE = (1 << 13); /*
                                                            * Role within the Organization
                                                            */

    public static final int VCARD_ATTRIB_LOGO = (1 << 14); /* Organization Logo */

    public static final int VCARD_ATTRIB_AGENT = (1 << 15); /*
                                                             * vCard of Person Representing
                                                             */

    public static final int VCARD_ATTRIB_ORG = (1 << 16); /*
                                                           * Name of Organization
                                                           */

    public static final int VCARD_ATTRIB_NOTE = (1 << 17); /* Comments */

    public static final int VCARD_ATTRIB_REV = (1 << 18); /* Revision */

    public static final int VCARD_ATTRIB_SOUND = (1 << 19); /*
                                                             * Pronunciation of Name
                                                             */

    public static final int VCARD_ATTRIB_URL = (1 << 20); /*
                                                           * Uniform Resource Locator
                                                           */

    public static final int VCARD_ATTRIB_UID = (1 << 21); /* Unique ID */

    public static final int VCARD_ATTRIB_KEY = (1 << 22); /*
                                                           * Public Encryption Key
                                                           */

    public static final int VCARD_ATTRIB_NICKNAME = (1 << 23); /* Nickname */

    public static final int VCARD_ATTRIB_CATEGORIES = (1 << 24); /* Categories */

    public static final int VCARD_ATTRIB_PROID = (1 << 25); /* Product ID */

    public static final int VCARD_ATTRIB_CLASS = (1 << 26); /* Class information */

    public static final int VCARD_ATTRIB_SORTSTRING = (1 << 27); /*
                                                                  * String used for sorting operations
                                                                  */

    public static final int VCARD_ATTRIB_XIRMCCALLDATETIME = (1 << 28); /*
                                                                         * Time stamp
                                                                         */
    ...
    private long mFilter = 0x0;

    public boolean movePosition(int position) {
    ...
    public void setVCardAttribFilter(long filter) {
        Log.d(LOG_TAG, "[API] setVCardAttribFilter(" + filter + ")");
        mFilter = filter;
    }

    private void appendTitles(final StringBuilder builder, final Map<String, List<ContentValues>> contentValuesListMap) {
        final List<ContentValues> contentValuesList = contentValuesListMap.get(Organization.CONTENT_ITEM_TYPE);
        if (contentValuesList != null) {
            for (ContentValues contentValues : contentValuesList) {
                ...
                if (title != null) {
                    ...
                    if (!TextUtils.isEmpty(title)) {
                        appendVCardLine(builder, VCARD_PROPERTY_TITLE, title, !VCardUtils.containsOnlyPrintableAscii(title),
                                (mUsesQuotedPrintable && !VCardUtils.containsOnlyNonCrLfPrintableAscii(title)));
                    }
                }
            }
        }
