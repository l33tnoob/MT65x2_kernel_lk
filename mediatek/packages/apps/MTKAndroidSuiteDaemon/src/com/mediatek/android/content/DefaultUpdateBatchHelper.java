...
public abstract class DefaultUpdateBatchHelper extends OperationBatchHelper {
    private boolean[] mUpdateResults;

    /**
     * @param opBatch
     *            The ContentProviderOperationBatch used to update.
     */
    public DefaultUpdateBatchHelper(final ContentProviderOperationBatch opBatch) {
        super(opBatch);
    }

    /**
     * @return a array of results for updating.
     */
    ...
    public String getName() {
        return "DefaultUpdateBatchHelper";
    };
    ...
    public void run(final int batchSize) {
        mUpdateResults = new boolean[batchSize];
        super.run(batchSize);
    }
    ...
    public void onOperationResult(final ContentProviderResult opResult,
        ...
        if (null == opResult) {
            ...
            mUpdateResults[executedPosition] = false;
        } else if (1 == opResult.count) {
            mUpdateResults[executedPosition] = true;
        } else {
            Debugger.logW(getName(), "onOperationResult", new Object[] {
                    opResult, executedPosition },
