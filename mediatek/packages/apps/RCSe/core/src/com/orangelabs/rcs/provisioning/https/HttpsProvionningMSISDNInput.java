...
public final class HttpsProvionningMSISDNInput {
    ...
    protected void displayErrorMessageAndWait(Context context) {
        Intent intent = new Intent(context, HttpsProvisioningAlertDialog.class);
        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
    ...
    protected void errorMsgResponseAcknowledged() {
        synchronized (HttpsProvionningMSISDNInput.instance) {
            super.notify();
        }
    }
    ...
    protected String displayOTPDialog(Context context) {
        Intent intent = new Intent(context, HttpsProvisioningAlertDialog.class);
        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        intent.putExtra("showOtpDialog", true);
        context.startActivity(intent);
        try {
            synchronized (HttpsProvionningMSISDNInput.instance) {
                super.wait();
    ...
    protected void updateOTPDialog(String Otp) {
        synchronized (HttpsProvionningMSISDNInput.instance) {
        	OTPCode = Otp;
            super.notify();
        }
