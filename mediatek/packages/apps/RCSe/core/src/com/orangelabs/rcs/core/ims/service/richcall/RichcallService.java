...
public class RichcallService extends ImsService {
    ...
    public void receiveCapabilityRequest(SipRequest options) {
	    ...
	    try {
			...
			if(richcall){
				if (logger.isActivated()) {
					logger.debug("OPTIONS options richcall is true");
				}
			}
		...
		if (logger.isActivated()) {
			logger.debug("options request received during a call from1 " + contact);
		}
    	...
    	if (capabilities.isImSessionSupported()) {
			if (logger.isActivated()) {
			logger.debug("options receiveCapabilityRequest IMS SESSION SUPPORTED");
		}
