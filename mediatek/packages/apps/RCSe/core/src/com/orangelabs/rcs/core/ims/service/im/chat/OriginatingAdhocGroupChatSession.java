...
public class OriginatingAdhocGroupChatSession extends GroupChatSession {
	 ...
	 */
	public OriginatingAdhocGroupChatSession(ImsService parent, String conferenceId, String subject , String firstMsg, ListOfParticipant participants) {
		super(parent, conferenceId, participants);

		// Set subject
		if ((subject != null) && (subject.length() > 0)) {
			String new_subject = subject + SipUtils.DELIMITER1 + firstMsg ;
			setSubject(new_subject);
            setMessageText(firstMsg);
            /**
             * M: add first message @{
             */
            InstantMessage firstMessage = ChatUtils.createFirstMessage(
                    getRemoteContact(), firstMsg, false);
