...
import com.hissage.vcard.NmsContactStruct.PhoneData;
...
public class VCardComposer {
     ...
     */
    public String createVCard(NmsContactStruct struct, int vcardversion) throws VCardException {
        ...
        /** modify by jhnie for Hissage sync **/
        if (struct.UID != null) {
            mResult.append("UID:" + struct.UID + mNewline);
        }

        if (!TextUtils.isEmpty(struct.others)) {
            mResult.append(struct.others);
        }

        if (struct.TimeStamp != null) {
            mResult.append("REV:" + struct.TimeStamp + mNewline);
        }
    ...
    private void appendPhoneStr(List<NmsContactStruct.PhoneData> phoneList, int version) {
        HashMap<String, String> numMap = new HashMap<String, String>();
        String joinMark = version == VERSION_VCARD21_INT ? ";" : ",";

        for (NmsContactStruct.PhoneData phone : phoneList) {
            String type;
    ...
    private void appendContactMethodStr(List<NmsContactStruct.ContactMethod> contactMList,
            int version) {

        HashMap<String, String> emailMap = new HashMap<String, String>();
        String joinMark = version == VERSION_VCARD21_INT ? ";" : ",";
        for (NmsContactStruct.ContactMethod contactMethod : contactMList) {
