 ...
 * primarily useful for code that wishes to be vendor-aware in its
 * interpretation of Japanese encoding names.
 * 
 * <p>
 * As of this writing, the only vendor that is recognized by this class is
 * Docomo (identified case-insensitively as {@code "docomo"}).
 * </p>
 * 
 * <b>Note:</b> This class is hidden in Cupcake, with a plan to un-hide in
 * Donut. This was done because the first deployment to use this code is based
 * on Cupcake, but the API had to be introduced after the public API freeze for
 * that release. The upshot is that only system applications can safely use this
 * class until Donut is available.
 * 
 * @hide
 */
public final class CharsetUtils {
    ...
    /**
     * Returns the vendor-specific character set corresponding to the given
     * original character set name and vendor. If there is no vendor-specific
     * character set for the given name/vendor pair, this returns the character
     * set corresponding to the original name. The vendor name is matched
     * case-insensitively. This method merely calls {@code Charset.forName()} on
     * a name transformed by a call to {@link #nameForVendor(String,String)}.
     * 
     * @param charsetName
     *            the base character set name
     * @param vendor
     *            the vendor to specialize for
     * @return the specialized character set, or the one corresponding directly
     *         to {@code charsetName} if there is no specialized variant
     * @throws UnsupportedCharsetException
     *             thrown if the named character set is not supported by the
     *             system
     * @throws IllegalCharsetNameException
     *             thrown if {@code charsetName} has invalid syntax
     */
    ...
    /**
     * Returns the vendor-specific character set corresponding to the given
     * original character set name and default vendor (that is, the targeted
     * vendor of the device this code is running on). This method merely calls
     * through to {@link #charsetForVendor(String,String)}, passing the default
     * vendor as the second argument.
     * 
     * @param charsetName
     *            the base character set name
     * @return the specialized character set, or the one corresponding directly
     ...
     *             thrown if {@code charsetName} has invalid syntax
     */
    public static Charset charsetForVendor(String charsetName) throws UnsupportedCharsetException,
