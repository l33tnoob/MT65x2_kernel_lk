...
/**
 * The parameter class of VCardComposer. This class standy by the person-contact
 * in Android system, we must use this class instance as parameter to transmit
 * to VCardComposer so that create vCard string.
 */
// TODO: rename the class name, next step
public class NmsContactStruct {

    // Note: phonetic name probably should be "LAST FIRST MIDDLE" for European
    // languages, and
    // space should be added between each element while it should not be in
    // Japanese.
    // But unfortunately, we currently do not have the data and are not sure
    // whether we should
    // support European version of name ordering.
    //
    // TODO: Implement the logic described above if we really need European
    // version of
    // phonetic name handling. Also, adding the appropriate test case of vCard
    // would be
    // highly appreciated.
    ...
    /** modify by jhnie, only for Hissage sync **/
    ...
    public static NmsContactStruct constructContactFromVNode(VNode node, int nameOrderType) {
        ...
        for (PropertyNode propertyNode : node.propList) {
            ...
            if (name.equals("VERSION")) {
                ...
                contact.addContactmethod(Contacts.KIND_EMAIL, type, propertyNode.propValue, label,
                        isPrimary);
    ...
    // }
    // } else {
    // personUri = provider.nonTransactionalInsert(People.CONTENT_URI,
    // contentValues);
    // if (personUri != null) {
    // personId = ContentUris.parseId(personUri);
    ...
    // Long.parseLong(organizationUri.getLastPathSegment());
    // }
    // }
    // }
    //
    ...
    // }
    //
    // if (primaryPhoneId >= 0 || primaryOrganizationId >= 0 || primaryEmailId
    // >= 0) {
    // ContentValues values = new ContentValues();
    // if (primaryPhoneId >= 0) {
