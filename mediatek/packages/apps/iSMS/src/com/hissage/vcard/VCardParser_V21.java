...
public class VCardParser_V21 {
    ...
    private static final HashSet<String> sKnownTypeSet = new HashSet<String>(Arrays.asList("DOM",
            "INTL", "POSTAL", "PARCEL", "HOME", "WORK", "PREF", "VOICE", "FAX", "MSG", "CELL",
            "PAGER", "BBS", "MODEM", "CAR", "ISDN", "VIDEO", "AOL", "APPLELINK", "ATTMAIL", "CIS",
            "EWORLD", "INTERNET", "IBMMAIL", "MCIMAIL", "POWERSHARE", "PRODIGY", "TLX", "X400",
            "GIF", "CGM", "WMF", "BMP", "MET", "PMB", "DIB", "PICT", "TIFF", "PDF", "PS", "JPEG",
            "QTIME", "MPEG", "MPEG2", "AVI", "WAVE", "AIFF", "PCM", "X509", "PGP"));

    /** Store the known-value */
    private static final HashSet<String> sKnownValueSet = new HashSet<String>(Arrays.asList(
            "INLINE", "URL", "CONTENT-ID", "CID"));

    /** Store the property names available in vCard 2.1 */
    private static final HashSet<String> sAvailablePropertyNameV21 = new HashSet<String>(
    ...
    private static final HashSet<String> sAvailableEncodingV21 = new HashSet<String>(Arrays.asList(
    ...
    protected boolean isValidPropertyName(String propertyName) {
        if (!(sAvailablePropertyNameV21.contains(propertyName.toUpperCase()) || propertyName
    ...
    // vCard 3.1 specification allows double-quoted param-value, while vCard 2.1
    // does not.
    ...
    protected void handlePropertyValue(String propertyName, String propertyValue)
        ...
        if (mEncoding.equalsIgnoreCase("QUOTED-PRINTABLE")) {
            ...
            try {
                ...
                // Log.e(LOG_TAG,
                // "OutOfMemoryError happened during parsing BASE64 data!");
                if (mBuilder != null) {
            ...
            if (!(mEncoding == null || mEncoding.equalsIgnoreCase("7BIT")
                    || mEncoding.equalsIgnoreCase("8BIT") || mEncoding.toUpperCase().startsWith(
                    "X-"))) {
                // Log.w(LOG_TAG, "The encoding unsupported by vCard spec: \"" +
                // mEncoding + "\".");
            }
    ...
    protected void handleMultiplePropertyValue(String propertyName, String propertyValue)
            throws IOException, VCardException {
        // vCard 2.1 does not allow QUOTED-PRINTABLE here, but some data have
        // it.
        if (mEncoding.equalsIgnoreCase("QUOTED-PRINTABLE")) {
    ...
    protected void handleAgent(String propertyValue) throws VCardException {
         ...
         * Ignore it for now. TODO: fix this.
         * 
         * String[] strArray = propertyValue.split(":", 2); if
         * (!(strArray.length == 2 ||
         * strArray[0].trim().equalsIgnoreCase("BEGIN") &&
         * strArray[1].trim().equalsIgnoreCase("VCARD"))) { throw new
         * VCardException("BEGIN:VCARD != \"" + propertyValue + "\""); }
    ...
    /**
     * Parse the given stream and constructs VCardDataBuilder object. Note that
     * vCard 2.1 specification allows "CHARSET" parameter, and some career sets
     * local encoding to it. For example, Japanese phone career uses Shift_JIS,
     * which is not formally allowed in vCard specification. As a result, there
     * is a case where the encoding given here does not do well with the
     * "CHARSET".
     * 
     * In order to avoid such cases, It may be fine to use "ISO-8859-1" as an
     * encoding, and to encode each localized String afterward.
     * 
     ...
     * @throws IOException
     */
    public boolean parse(InputStream is, String charset, VBuilder builder) throws IOException,
            VCardException {
        // TODO: make this count error entries instead of just throwing
        // VCardException.
    ...
    /**
     * It is very, very rare case, but there is a case where canceled may be
     * already true outside this object.
     * 
     * @hide
     */
